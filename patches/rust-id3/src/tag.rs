...
impl<'a> Tag {
    ...
    /// possible.
    pub fn write_to_with_base(&self, mut base: &mut std::io::Cursor<Vec<u8>>, version: Version) -> crate::Result<()> {
        #[allow(clippy::reversed_empty_ranges)]
        let location = storage::locate_id3v2(&mut base)?.unwrap_or(0..0); // Create a new tag if none could be located.

        let mut storage = PlainStorage::new(base, location);
